...
export async function fetchPublicReport(
  ...
  if (!response.ok) {
    throw new Error(
      data?.message || "Não foi possível carregar os resultados.",
    );
  }
...
export async function refreshPublicReport(): Promise<PublicReport> {
  ...
  if (!response.ok) {
    throw new Error(
      data?.message || "Não foi possível atualizar os aprendizados.",
    );
  }
...
export async function regenerateQuestionInsights(
  questionId: string,
